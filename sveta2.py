import asyncio
import config
import aiohttp  # Импорт aiohttp для выполнения HTTP-запросов
...
async def get_fox_image():
    url = "https://some-random-api.ml/img/fox"  # Используйте API, который предоставляет изображения лисиц
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            if response.status == 200:
                data = await response.json()
                return data.get("link")  # Здесь 'link' — это ключ, который содержит ссылку на изображение
            else:
                return None
async def main():
